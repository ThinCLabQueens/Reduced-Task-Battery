...
if __name__ == "__main__":
    ...
    with open("Analysis/accuracy.csv", "a") as f:
        newdict = {
            "Subject": None,
            "Experience Sampling Questions Response Time": None,
            "GoNoGo Task Response Time": None,
            "Go Task Accuracy": None,
            "NoGo Task Accuracy": None,
            "Finger Tapping Task Response Time": None,
            "Finger Tapping Task Accuracy": None,
            "Two-Back Task-faces Response Time": None,
            "Two-Back Task-faces Accuracy": None,
            "Two-Back Task-scenes Response Time": None,
            "Two-Back Task-scenes Accuracy": None,
            "One-Back Task Response Time": None,
            "One-Back Task Accuracy": None,
            "Zero-Back Task Response Time": None,
            "Zero-Back Task Accuracy": None,
            "Hard Math Task Response Time": None,
            "Hard Math Task Accuracy": None,
            "Easy Math Task Response Time": None,
            "Easy Math Task Accuracy": None,
            "Friend Task Response Time": None,
            "Friend Task Sentiment": None,
            "You Task Response Time": None,
            "You Task Sentiment": None,
        }
        writer = csv.writer(f)
    ...
    for file in os.listdir("Tasks/log_file"):
        ...
        if not "full" in ftemp.split("_"):
                ...
                for row in reader:
                    ...
                    if row[0] == "ESQ":
                        ...
                        if enum == 16:
                            ...
                            (
                                line_dict["Gradient 1"],
                                line_dict["Gradient 2"],
                                line_dict["Gradient 3"],
                            ) = grads
                ...
                for row in reader:
                    ...
                    # Trigger start on next line
                        ...
                        if expdict["Experiment"] == "Two-Back Task":
                            sortingfunction(
                                expdict["Experiment"] + "-" + row[9], row, resps
                            )
                        else:
            ...
            with open("Analysis/accuracy.csv", "a", newline="") as f:
                newdict = {
                    "Subject": resps["Subject"],
                    "Experience Sampling Questions Response Time": np.mean(
                        resps["Experience Sampling Questions"]["Response Time"]
                    ),
                    "GoNoGo Task Response Time": np.mean(
                        resps["GoNoGo Task"]["Response Time"]
                    )
                    if resps["GoNoGo Task"]["Response Time"] != []
                    else "nan",
                    "NoGo Task Accuracy": (
                        resps["GoNoGo Task"]["Accuracy - NoGo"].count(True)
                        / len(resps["GoNoGo Task"]["Accuracy - NoGo"])
                    )
                    if resps["GoNoGo Task"]["Accuracy - NoGo"] != []
                    else "nan",
                    "Go Task Accuracy": (
                        resps["GoNoGo Task"]["Accuracy - Go"].count(True)
                        / len(resps["GoNoGo Task"]["Accuracy - Go"])
                    )
                    if resps["GoNoGo Task"]["Accuracy - NoGo"] != []
                    else "nan",
                    "Finger Tapping Task Response Time": np.mean(
                        resps["Finger Tapping Task"]["Response Time"]
                    )
                    if resps["Finger Tapping Task"]["Response Time"] != []
                    else "nan",
                    "Finger Tapping Task Accuracy": (
                        resps["Finger Tapping Task"]["Accuracy"].count(True)
                        / len(resps["Finger Tapping Task"]["Accuracy"])
                    )
                    if resps["Finger Tapping Task"]["Accuracy"] != []
                    else "nan",
                    "Two-Back Task-faces Response Time": np.mean(
                        resps["Two-Back Task-faces"]["Response Time"]
                    )
                    if resps["Two-Back Task-faces"]["Response Time"] != []
                    else "nan",
                    "Two-Back Task-faces Accuracy": (
                        resps["Two-Back Task-faces"]["Accuracy"].count(True)
                        / len(resps["Two-Back Task-faces"]["Accuracy"])
                    )
                    if resps["Two-Back Task-faces"]["Accuracy"] != []
                    else "nan",
                    "Two-Back Task-scenes Response Time": np.mean(
                        resps["Two-Back Task-scenes"]["Response Time"]
                    )
                    if resps["Two-Back Task-scenes"]["Response Time"] != []
                    else "nan",
                    "Two-Back Task-scenes Accuracy": (
                        resps["Two-Back Task-scenes"]["Accuracy"].count(True)
                        / len(resps["Two-Back Task-scenes"]["Accuracy"])
                    )
                    if resps["Two-Back Task-scenes"]["Accuracy"] != []
                    else "nan",
                    "One-Back Task Response Time": np.mean(
                        resps["One-Back Task"]["Response Time"]
                    )
                    if resps["One-Back Task"]["Response Time"] != []
                    else "nan",
                    "One-Back Task Accuracy": (
                        resps["One-Back Task"]["Accuracy"].count(True)
                        / len(resps["One-Back Task"]["Accuracy"])
                    )
                    if resps["One-Back Task"]["Accuracy"] != []
                    else "nan",
                    "Zero-Back Task Response Time": np.mean(
                        resps["Zero-Back Task"]["Response Time"]
                    )
                    if resps["Zero-Back Task"]["Response Time"] != []
                    else "nan",
                    "Zero-Back Task Accuracy": (
                        resps["Zero-Back Task"]["Accuracy"].count(True)
                        / len(resps["Zero-Back Task"]["Accuracy"])
                    )
                    if resps["Zero-Back Task"]["Accuracy"] != []
                    else "nan",
                    "Hard Math Task Response Time": np.mean(
                        resps["Hard Math Task"]["Response Time"]
                    )
                    if resps["Hard Math Task"]["Response Time"] != []
                    else "nan",
                    "Hard Math Task Accuracy": (
                        resps["Hard Math Task"]["Accuracy"].count(True)
                        / len(resps["Hard Math Task"]["Accuracy"])
                    )
                    if resps["Hard Math Task"]["Accuracy"] != []
                    else "nan",
                    "Easy Math Task Response Time": np.mean(
                        resps["Easy Math Task"]["Response Time"]
                    )
                    if resps["Easy Math Task"]["Response Time"] != []
                    else "nan",
                    "Easy Math Task Accuracy": (
                        resps["Easy Math Task"]["Accuracy"].count(True)
                        / len(resps["Easy Math Task"]["Accuracy"])
                    )
                    if resps["Easy Math Task"]["Accuracy"] != []
                    else "nan",
                    "Friend Task Response Time": np.mean(
                        resps["Friend Task"]["Response Time"]
                    )
                    if resps["Friend Task"]["Response Time"] != []
                    else "nan",
                    "Friend Task Sentiment": (
                        resps["Friend Task"]["Sentiment"].count(True)
                        / len(resps["Friend Task"]["Sentiment"])
                    )
                    if resps["Friend Task"]["Sentiment"] != []
                    else "nan",
                    "You Task Response Time": np.mean(resps["You Task"]["Response Time"])
                    if resps["You Task"]["Response Time"] != []
                    else "nan",
                    "You Task Sentiment": (
                        resps["You Task"]["Sentiment"].count(True)
                        / len(resps["You Task"]["Sentiment"])
                    )
                    if resps["You Task"]["Sentiment"] != []
                    else "nan",
                }
                writer = csv.writer(f)
